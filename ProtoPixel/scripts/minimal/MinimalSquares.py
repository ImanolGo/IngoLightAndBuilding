...
class Square:
    ...
    def setup(self):
        ...
        r1 = randint(0,len(rows) - 1)
        c1 = randint(0,len(cols) - 1)

        r2 = randint(0,len(rows) - 1)
        c2 = randint(0,len(cols) - 1)

        self.yPos = min(rows[r1],rows[r2])*self.size - self.thickness*0.5
        self.xPos = min(cols[c1],cols[c2])*self.size - self.thickness*0.5
        self.height = abs(rows[r1] - rows[r2])*self.size + self.thickness
        self.width = abs(cols[c1] - cols[c2])*self.size + self.thickness
