...
def update():
    ...
    global elapsedTime, color, startColorIndex, endColorIndex, brightness, currentSpeed, speedFactor, scaleFactor

    currentSpeed = currentSpeed + ( content['speed'] - currentSpeed ) * 0.9

    t = (ofGetElapsedTimef()) * currentSpeed*speedFactor
    brightness = 255*(ofNoise(t)>0.5)
